...
class AIStudio(QMainWindow):
    ...
    def create_dashboard(self) -> QWidget:
        ...
        quick_grid.addWidget(
            self._create_action_card(
                "🎬",
                "AI Director",
                "Tạo kịch bản, chia cảnh, shot và prompt Veo.",
                self._show_ai_director,
            ),
            0,
            0,
        )
        quick_grid.addWidget(
            self._create_action_card(
                "🧩",
                "Storyboard",
                "Quản lý shot, asset, thời lượng và timeline.",
        ...
        quick_grid.addWidget(
            self._create_action_card(
                "⚡",
                "Render Queue",
                "Xếp hàng, chạy tuần tự, tạm dừng và tiếp tục.",
                self._show_render_queue,
            ),
            1,
            0,
        )
        quick_grid.addWidget(
            self._create_action_card(
                "🎙",
                "Voice Studio",
                "Tạo giọng đọc từng cảnh và xuất phụ đề SRT.",
                self._show_voice_studio,
            ),
            1,
            1,
        )
        quick_grid.addWidget(
            self._create_action_card(
                "🎵",
                "Music & SFX",
                "Quản lý nhạc nền, hiệu ứng, âm lượng và fade.",
                self._show_music_studio,
            ),
            2,
            0,
        )
    ...
    def _show_ai_director(self) -> None:
        from app.ai_director_page import AIDirectorPage

        page = self._get_or_create_page("ai_director", AIDirectorPage)
        if hasattr(page, "back_requested"):
            try:
                page.back_requested.disconnect(self.show_dashboard)
            except (RuntimeError, TypeError):
                pass
            page.back_requested.connect(self.show_dashboard)

        self.pages.setCurrentWidget(page)
        self.set_active_menu("ai_director")
        self.statusBar().showMessage("AI Director đã sẵn sàng", 3000)

    def _show_storyboard(self) -> None:
        from app.storyboard_page import StoryboardPage

        page = self._get_or_create_page("storyboard", StoryboardPage)
        if hasattr(page, "back_requested"):
            try:
                page.back_requested.disconnect(self.show_dashboard)
            except (RuntimeError, TypeError):
        ...
        self.pages.setCurrentWidget(page)
        self.set_active_menu("storyboard")
        self.statusBar().showMessage("Storyboard đã sẵn sàng", 3000)

    def _show_render_queue(self) -> None:
        from app.render_queue_page import RenderQueuePage

        page = self._get_or_create_page("render_queue", RenderQueuePage)
        if hasattr(page, "back_requested"):
            try:
                page.back_requested.disconnect(self.show_dashboard)
            except (RuntimeError, TypeError):
        ...
        self.pages.setCurrentWidget(page)
        self.set_active_menu("render_queue")
        self.statusBar().showMessage("Render Queue đã sẵn sàng", 3000)

    def _show_voice_studio(self) -> None:
        from app.voice_studio_page import VoiceStudioPage

        page = self._get_or_create_page("voice_studio", VoiceStudioPage)
        if hasattr(page, "back_requested"):
            try:
                page.back_requested.disconnect(self.show_dashboard)
            except (RuntimeError, TypeError):
        ...
        self.pages.setCurrentWidget(page)
        self.set_active_menu("voice_studio")
        self.statusBar().showMessage("Voice Studio đã sẵn sàng", 3000)

    def _show_music_studio(self) -> None:
        from app.music_studio_page import MusicStudioPage

        page = self._get_or_create_page("music_studio", MusicStudioPage)
        if hasattr(page, "back_requested"):
            try:
                page.back_requested.disconnect(self.show_dashboard)
            except (RuntimeError, TypeError):
        ...
        self.pages.setCurrentWidget(page)
        self.set_active_menu("music_studio")
        self.statusBar().showMessage("Music Studio đã sẵn sàng", 3000)


    def _show_timeline(self) -> None:
        from app.timeline_editor_page import TimelineEditorPage

        page = self._get_or_create_page("timeline", TimelineEditorPage)
        if hasattr(page, "back_requested"):
            try:
                page.back_requested.disconnect(self.show_dashboard)
            except (RuntimeError, TypeError):
        ...
        self.pages.setCurrentWidget(page)
        self.set_active_menu("timeline")
        self.statusBar().showMessage("Timeline Engine đã sẵn sàng", 3000)
    ...
    def change_page(self, page_key: str) -> None:
        if page_key == "dashboard":
            ...
            self.show_settings()
        elif page_key == "ai_director":
            self._show_ai_director()
        elif page_key == "film_studio":
            self._show_film_studio()
        elif page_key == "storyboard":
            self._show_storyboard()
        elif page_key == "render_queue":
            self._show_render_queue()
        elif page_key == "voice_studio":
            self._show_voice_studio()
        elif page_key == "music_studio":
            self._show_music_studio()

...
class SceneCard(QFrame):
    ...
    Lưu ý về ảnh:
    - QPixmap chỉ được tạo và gán trong GUI thread.
    - Nếu worker gọi display_image(), đường dẫn ảnh sẽ được
      chuyển về GUI thread qua signal queued.
    """
    ...
    _display_image_in_gui = Signal(str)
    _display_voice_in_gui = Signal(str)
    _display_video_in_gui = Signal(str)
    ...
    def __init__(
        ...
        self._display_image_in_gui.connect(
            self._apply_image,
            Qt.ConnectionType.QueuedConnection,
        )
        self._display_voice_in_gui.connect(
            self._apply_voice,
            Qt.ConnectionType.QueuedConnection,
        )
        self._display_video_in_gui.connect(
            self._apply_video,
            Qt.ConnectionType.QueuedConnection,
        )
    ...
    def display_image(
        ...
        Có thể được gọi từ worker thread. Nếu không ở GUI thread,
        hàm chỉ phát signal để xử lý ở GUI thread.
        """
        ...
        if (
            ...
            self._display_image_in_gui.emit(
                normalized_path
            )
            return
    ...
    def _apply_image(
        ...
        if (
            ...
            self.image_preview.clear()
            self.image_preview.setText(
                "Không tìm thấy hoặc ảnh không hợp lệ."
            )
            return
        ...
        if pixmap.isNull():
            self.image_preview.clear()
            self.image_preview.setText(
                "Không thể chuyển ảnh để hiển thị."
            )
            return
    ...
    def display_voice(
        ...
        if (
            ...
            self._display_voice_in_gui.emit(
                normalized_path
            )
            return
    ...
    def display_video(
        ...
        if (
            ...
            self._display_video_in_gui.emit(
                normalized_path
            )
            return
